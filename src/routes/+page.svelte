<script lang="ts">
	...
	const qualifiedForFreeShipping = $derived(cartStats.total >= 50);
	
	$effect(() => {
		if(qualifiedForFreeShipping) {
			alert('You have qualified for a free shipping!');
		}
	})
...
<div class="bg-gray-3000 flex items-center p-4">
		...
		{#if cartOpen}
			<div class="absolute right-0 top-8 z-10 mt-2 w-80 rounded-lg bg-white shadow-xl">
				<div class="relative p-4">
					...
					aria-label="Close cart" onclick="{() => cartOpen = false}">
						<X class="size-4" />
					</button>
